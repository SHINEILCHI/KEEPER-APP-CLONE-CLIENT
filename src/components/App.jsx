...
function App(props) {
  ...
  useEffect(() => {
    Axios.get("https://salty-savannah-57698.herokuapp.com/").then((response) => {
      setNoteItems(response.data)
    })
  }, [])

  async function refreshPage() {
    const response = await Axios.get("https://salty-savannah-57698.herokuapp.com/");
  ...
  async function addNote(inputContent, setInputContent) {
    const response = await Axios.post("https://salty-savannah-57698.herokuapp.com/createNote", {
  ...
  async function deleteNote(_id) {
    const response = await Axios.delete("https://salty-savannah-57698.herokuapp.com/deleteNote", {params: { _id }});
    if (response.status === 200) {
      setNoteItems((prevNoteItems) => {
